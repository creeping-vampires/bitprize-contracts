...
contract DeployPrizePool is ScriptBase {
    ...
    function run() public virtual {
        ...
        if (
            keccak256(bytes(config.rngType)) ==
            keccak256("witnet-randomness-v2")
        ) {
            /// WITNET
            standardizedRng = new RngWitnet(IWitnetRandomness(config.rng));
        } else if (
            keccak256(bytes(config.rngType)) == keccak256("standardized")
        ) {
            /// STANDARDIZED
    ...
    function deployCore() public {
        ...
        stakingVault = new StakingVault(
            config.stakingVaultName,
            config.stakingVaultSymbol,
            StakingVaultIERC20(config.stakedAsset)
        );
